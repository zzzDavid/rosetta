...
void integralImages(int height, int width,
                    unsigned char Data[IMAGE_HEIGHT][IMAGE_WIDTH],
                    int Sum[IMAGE_HEIGHT][IMAGE_WIDTH],
                    int Sqsum[IMAGE_HEIGHT][IMAGE_WIDTH]);

void imageScaler(int src_height, int src_width,
                 unsigned char Data[IMAGE_HEIGHT][IMAGE_WIDTH], int dest_height,
                 int dest_width,
                 unsigned char IMG1_data[IMAGE_HEIGHT][IMAGE_WIDTH]);

void processImage(float factor, int sum_row, int sum_col, int *AllCandidates_x,
                  int *AllCandidates_y, int *AllCandidates_w,
                  int *AllCandidates_h, int *AllCandidates_size,
                  unsigned char IMG1_data[IMAGE_HEIGHT][IMAGE_WIDTH],
                  MySize winSize);

int cascadeClassifier(int SUM1_data[IMAGE_HEIGHT][IMAGE_WIDTH],
                      int SQSUM1_data[IMAGE_HEIGHT][IMAGE_WIDTH], MyPoint pt);

int weakClassifier(int stddev, int coord[12], int haar_counter, int w_id);

void groupRectangles(MyRect *rectList, int *rectList_size, int groupThreshold,
                     float eps);
...
void face_detect_sw
    ...
    (unsigned char Data[IMAGE_HEIGHT][IMAGE_WIDTH], int result_x[RESULT_SIZE],
     int result_y[RESULT_SIZE], int result_w[RESULT_SIZE],
     int result_h[RESULT_SIZE], int *result_size)
...
L1:
  while (IMAGE_WIDTH / factor > WINDOW_SIZE &&
         IMAGE_HEIGHT / factor > WINDOW_SIZE) { // condition can be evaluated
    ...
    imageScaler(IMAGE_HEIGHT, IMAGE_WIDTH, Data, height, width, IMG1_data);

    processImage(factor, height, width, result_x, result_y, result_w, result_h,
                 result_size, IMG1_data, winSize);
    factor *= scaleFactor;
  }
}
...
void processImage
    ...
    (float factor, int sum_row, int sum_col, int *AllCandidates_x,
     int *AllCandidates_y, int *AllCandidates_w, int *AllCandidates_h,
     int *AllCandidates_size,
     unsigned char IMG1_data[IMAGE_HEIGHT][IMAGE_WIDTH], MySize winSize) {
  MyPoint p;
  int result;
  int step;
...
Pixely:
  for (y = 0; y < sum_row - WINDOW_SIZE + 1; y++) { // loop upper bound can be evaluated during interpretation
  Pixelx:
    for (x = 0; x < sum_col - WINDOW_SIZE + 1; x++) { // loop upper bound can be evaluated during interpretation
      ...
      if (result > 0) {
        MyRect r = {myRound(p.x * factor), myRound(p.y * factor), winSize.width,
                    winSize.height};
...
#include "haar_dataRcc_with_partitioning.h"
  ...
  stddev = SQSUM1_data[pt.y][pt.x] - SQSUM1_data[pt.y][pt.x + WINDOW_SIZE - 1] -
           SQSUM1_data[pt.y + WINDOW_SIZE - 1][pt.x] +
           SQSUM1_data[pt.y + WINDOW_SIZE - 1][pt.x + WINDOW_SIZE - 1];

  mean = SUM1_data[pt.y][pt.x] - SUM1_data[pt.y][pt.x + WINDOW_SIZE - 1] -
  ...
  if (stddev > 0) { // THIS CONDITION IS DATA DEPENDENT
...
Stages:
  for (i = 0; i < 25; i++) {
  Filters:
    for (j = 0; j < stages_array[i]; j++) { // upper bound is a constant included from harr_dadtaRcc_with_partitioning.h

      if (j == 0) { // this condition can be evaluted during interpretation
        stage_sum = 0;
        s = 0;
      }
      ...
      if (!(tr2.x == 0 && tr2.width == 0 && tr2.y == 0 && tr2.height == 0) &&
          tr2.width != 0 && tr2.height != 0) {
        ...
        coord[11] =
            SUM1_data[pt.y + tr2.y + tr2.height][pt.x + tr2.x + tr2.width];
      } else {
        coord[8] = 0;
        coord[9] = 0;
        coord[10] = 0;
...
#include "haar_dataEWC_with_partitioning.h"
  ...
  sum0 = (coord[0] - coord[1] - coord[2] + coord[3]) *
         weights_array0[haar_counter]; //[w_id] area of 1st filter block
                                       //(rectangle) multiplied by its weigh
  sum1 = (coord[4] - coord[5] - coord[6] + coord[7]) *
         weights_array1[haar_counter]; //[w_id+1];
  sum2 = (coord[8] - coord[9] - coord[10] + coord[11]) *
         weights_array2[haar_counter]; //[w_id+2];
  final_sum = sum0 + sum1 + sum2;

  if (final_sum >= t) { // THIS CONDITION IS DATA DEPENDENT
